...
Lets view the first 5 rows of data: 
```{python}
iris.head()
```

Now, lets view the last 5 rows of data:
```{python}
iris.tail()
```
Great! Now we know what our data looks like and the column names. However, I want to make sure the data format is okay. To do so, I am going to confirm the data types of iris using the .dtypes. function. We can now see that the columns are all floats except the species column. This looks all good!
